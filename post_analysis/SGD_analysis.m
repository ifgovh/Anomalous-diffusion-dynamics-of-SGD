...
for ii = 1:length(d)
    ...
    legend_txt = [];
    for jj = 1:length(t_start)
        plot(Displacement_all(t_start(jj),(1 + t_start(jj)):end)./(mean(GN(t_start(jj):end,:),2).^2)');
        legend_txt{end+1} = ['t_w=',num2str(t_start(jj))];
    end
    set(gca,'xscale','log','yscale','log')
    xlabel('\tau')
    ...
    legend_txt = [];
    for jj = 1:length(t_start)
        plot(Contour_length_all(t_start(jj),2:end),Displacement_all(t_start(jj),2:end)./(mean(GN,2).^2)');        
        legend_txt{end+1} = ['t_w=',num2str(t_start(jj))];
    end
    set(gca,'xscale','log','yscale','log')
    xlabel('Contour length')
