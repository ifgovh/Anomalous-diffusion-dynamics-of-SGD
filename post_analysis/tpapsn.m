...
% reference: built-in function and ARGUS stochastic search method
% modified by Guozhang Chen, 06/05/2020
...
if radiags(1)<1.e-14*radiags(end)
    ...
    st.centers = x.'; % make the data shape same as the built-in function's output
    st.coefs =[coefs1,coefs2];
    [dco, nco] = size(st.coefs);
    st.ncenters = nx;
    st.number = nco;
    st.dim = dco;
    st.form = 'st-tp00'; % make the data shape same as the built-in function's output
    ...
    if nargin<4||isempty(interv)
        ...
        if nx==1
            interv = {st.centers(1,1)+[0 1], st.centers(2,1)+[0 1]};
        else
            for j=m:-1:1
                interv{j} = [min(st.centers(j,:)), max(st.centers(j,:))];
            end
        end
        st.interv = interv;
    else
